...
// have no records.
if(_internalModel2.hasScheduledDestroy()){internalModels[_i2].cancelDestroy();}}function _fetchRecord(recordResolverPair){var recordFetch=store._fetchRecord(recordResolverPair.internalModel,recordResolverPair.options); // TODO adapter options
...
var found=Object.create(null);for(var _i3=0,_l=foundInternalModels.length;_i3 < _l;_i3++) {var _internalModel3=foundInternalModels[_i3];var _pair=seeking[_internalModel3.id];found[_internalModel3.id] = _internalModel3;if(_pair){var resolver=_pair.resolver;resolver.resolve(_internalModel3);}} // reject missing records
var missingInternalModels=[];for(var _i4=0,_l2=expectedInternalModels.length;_i4 < _l2;_i4++) {var _internalModel4=expectedInternalModels[_i4];if(!found[_internalModel4.id]){missingInternalModels.push(_internalModel4);}}if(missingInternalModels.length){true && Ember.warn('Ember Data expected to find records with the following ids in the adapter response but they were missing: ' + Ember.inspect(missingInternalModels.map(function(r){return r.id;})),false,{id:'ds.store.missing-records-from-adapter'});rejectInternalModels(missingInternalModels);}}function rejectInternalModels(internalModels,error){for(var _i5=0,_l3=internalModels.length;_i5 < _l3;_i5++) {var _internalModel5=internalModels[_i5];var _pair2=seeking[_internalModel5.id];if(_pair2){_pair2.resolver.reject(error || new Error('Expected: \'' + _internalModel5 + '\' to be present in the adapter provided payload, but it was not found.'));}}}if(shouldCoalesce){ // TODO: Improve records => snapshots => records => snapshots
//
...
var snapshots=new Array(totalItems);for(var _i6=0;_i6 < totalItems;_i6++) {snapshots[_i6] = internalModels[_i6].createSnapshot();}var groups=adapter.groupRecordsForFindMany(this,snapshots);for(var i=0,l=groups.length;i < l;i++) {var group=groups[i];var totalInGroup=groups[i].length;var ids=new Array(totalInGroup);var groupedInternalModels=new Array(totalInGroup);for(var j=0;j < totalInGroup;j++) {var internalModel=group[j]._internalModel;groupedInternalModels[j] = internalModel;ids[j] = internalModel.id;}if(totalInGroup > 1){(function(groupedInternalModels){_findMany(adapter,store,modelName,ids,groupedInternalModels).then(function(foundInternalModels){handleFoundRecords(foundInternalModels,groupedInternalModels);})['catch'](function(error){rejectInternalModels(groupedInternalModels,error);});})(groupedInternalModels);}else if(ids.length === 1){var pair=seeking[groupedInternalModels[0].id];_fetchRecord(pair);}else {true && !false && Ember.assert("You cannot return an empty array from adapter's method groupRecordsForFindMany",false);}}}else {for(var _i7=0;_i7 < totalItems;_i7++) {_fetchRecord(pendingFetchItems[_i7]);}}}, /**
